                ...
                <tbody>
                    @foreach ($orders as $order)
                        <tr class="border-b py-3">
                            <td class="py-1 px-2">
                                <a href="{{ route('order.view', $order) }}" class="text-cyan-600 hover:text-cyan-500">
                                    {{ $order->id }}
                                </a>
                            </td>
                            ...
                            <td class="py-1 px-2">
                                ...
                                {{ $order->isPaid() ? 'bg-emerald-500' : 'bg-slate-400' }}">
                                    {{ $order->status }}
                                </small>
                            </td>
                            ...
                            <td class="py-1 px-2 flex gap-2 w-[100px]">
                                @if (!$order->isPaid())
                                    <form action="{{ route('cart.checkout-order', $order) }}" method="POST">
                                        @csrf
                                        <button
                                            class="flex items-center py-1 px-2 bg-indigo-600 text-white duration-100 hover:bg-indigo-700 hover:ring-offset-2 hover:ring-2 hover:ring-offset-indigo-600 rounded-md whitespace-nowrap">
